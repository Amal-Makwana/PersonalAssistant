...
export class EventsRepository {
  ...
  private mapEventRowToContract(
    row: DbEventRow,
    reminderRows: DbReminderPlanRow[],
  ...
  async getAllEvents(): Promise<EventsResponse> {
    ...
    return {
      events: result.rows.map((row) =>
        this.mapEventRowToContract(row, remindersByEventId.get(row.id) ?? [], syncStatuses.get(row.id))
      )
    };
  }
  ...
  async getNotificationHistory(eventId: string): Promise<NotificationHistoryResponse | null> {
    const eventLookup = await query<{ id: string }>(
      'SELECT id FROM events WHERE id = $1::uuid AND deleted_at IS NULL LIMIT 1',
      [eventId]
    );
